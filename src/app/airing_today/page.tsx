...
function AiringToday() {
    ...
    useEffect(() => {
        const loadTvAiringToday = async () => {
            try {
                const { tvSeries: fetchedTv, totalPages: fetchedTotalPages } = await fetchTv(currentPage, 'airing_today');
    ...
    return (
        <div>
            <div className="my-16 p-4 flex flex-col items-center" ref={topRatedRef}>
                <h2 className="text-3xl font-bold mb-4 text-center">Airing Today</h2>
                {loading ? (
                    <Skeleton/>
                ) : (
